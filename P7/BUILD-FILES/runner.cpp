...
namespace FastIO {
    struct Reader {
        ...
        public:
            ...
            inline int readInt() { return getInt<int>(); }
            inline long long int readLong() { return getInt<long long int>(); }

            vector<int> readIntArray(int n) {
                vector<int> res;
                for (int i = 0; i < n; i++) {
                    res.push_back(readInt());
                }
                return res;
            }
    };
...
#undef PRINT_PART
    ...
    template<typename T>
        void Writer::printArray(const vector<T>& a) {
            int n = a.size();
            for (int i = 0; i < n - 1; i++) {
                (*this) << a[i] << " ";
            }
...
int main() {
    ...
    FastIO::Writer cout;
    int n = cin.readInt();
    long long int k = cin.readLong();
    vector<int> buy = cin.readIntArray(n);
    vector<int> sell = cin.readIntArray(n);
    cout.printArray(Stocks(buy, sell, k));
}
